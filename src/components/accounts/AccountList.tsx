...
const AccountList = () => {
  ...
  const { data: accounts, loading, error, remove, refetch } = useSupabaseData('accounts', user?.id);
              ...
              <CardContent className="space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {Math.abs(displayBalance - (Number(account.balance) || 0)) > 0.01 && (
                      <div className="text-xs text-orange-600">
                        <p>Calculado: {formatCurrency(displayBalance)}</p>
                        <p>Registrado: {formatCurrency(Number(account.balance) || 0)}</p>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="flex space-x-2">
                  ...
                  <AccountTransfer
                    ...
                    fromAccountBalance={displayBalance}
                    onTransferComplete={handleTransactionAdded}
                  />
                </div>
              </CardContent>
            </Card>
          ...
          <div className="overflow-y-auto">
            {selectedAccount && (
              <AccountBalanceHistory
                ...
                currentBalance={calculateAccountBalance(selectedAccount.id)}
              />
            )}
          </div>
        </DialogContent>
